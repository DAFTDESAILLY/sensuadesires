   ...
   /*vistas de mensaje izq */
    inner class ViewHolder(itemView : View): RecyclerView.ViewHolder(itemView){
       ...
       init {
           ...
           imagen_enviada_izquierda = itemView.findViewById(R.id.imagen_enviada_izquierda)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, position: Int): ViewHolder {
       return  if (position ==1){
           val view :View = LayoutInflater.from(contexto).inflate(com.desailly.chat_sensuadesires.R.layout.item_mensaje_derecho,parent,false)
            ViewHolder(view)
       }else{
           val view : View = LayoutInflater.from(contexto).inflate(com.desailly.chat_sensuadesires.R.layout.item_mensaje_izquierdo,parent,false)
            ViewHolder(view)
       }
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
       val chat : Chat = chatLista[position]
        /*posible cambio*/
        Glide.with(contexto).load(imagenUrl).placeholder(R.drawable.ic_imagenchat).into(holder.imagen_perfil_mensaje!!)
            /*si el mensaje contiene una imagen*/
        if (chat.getMensaje().equals("Se ha envio la imagen")&& !chat.getUrl().equals("")){
           /*usuario envia como mensaje */
            ...
            /*usuario nos envia una imagen como mensaje */
                ...
                holder.TXT_ver_mensaje!!.visibility = View.GONE
                holder.imagen_enviada_izquierda!!.visibility= View.VISIBLE
                Glide.with(contexto).load(chat.getUrl()).placeholder(R.drawable.ic_imagen_enviada).into(holder.imagen_enviada_izquierda!!)
            }
        }
        /* contiene solo texto */
        else{
